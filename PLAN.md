...
- **Coin-Grouped Coverage View**: Replaced the separate Live and Historical tabs in SystemPanel with a unified Coverage tab. Pairs are grouped by base asset (BTC, ETH, etc.) in collapsible accordion cards. Each card header shows live count, backfill progress segments, and expand/collapse. Inner table shows per-quote price, feed health, 1h/1d backfill badges, earliest data, and source. Consumes existing `/tickers/stream` and `/system/tasks/stream` SSE for live updates with no backend changes.
...
- [x] **Operations Panel Overhaul**: Split the monolithic `SystemPanel` into three focused tabs — Overview (warnings + version), Coverage (coin-grouped live + historical status with collapsible cards per base asset), and Audit (task tables). The Coverage tab merges the former Live and Historical tabs into a single view grouped by coin (e.g., BTC shows BTCEUR and BTCUSD together), displaying live feed health, backfill resolution badges, earliest data, and source per quote variant. Enriched the task SSE stream with `syncStatus` so coverage data updates in real time.
